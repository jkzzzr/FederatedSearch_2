...
public class IndexOnDisk extends Index {
	...
	protected IndexOnDisk(String _path, String _prefix, boolean isNew) {
		...
		this.path = _path;
		this.prefix = _prefix;
//		System.out.println("@IndexOnDisk - IndexOnDisk\t!"+this.path+"\t!"+this.prefix+"\tend");
	...
	/**
	 * 加载所有索引结构。如果索引属性<tt>index.preloadIndics.disabled<tt>为证的话，就不能这样做了
	 * 默认为假，意味着将初始化所有无输入流的索引。就直接退出函数
	 * 如果为真的话，就按要求加载
	...
	/**
	 * 对于不变的索引，使用正常的集合统计信息，从不更改<br>（像是初始化structureCache对象）
	...
	/**
	 * 对于尚未构建的索引，使用UpdatingCollectionStatistics，这种方法更慢，但是可以支持索引统计信息的更新
