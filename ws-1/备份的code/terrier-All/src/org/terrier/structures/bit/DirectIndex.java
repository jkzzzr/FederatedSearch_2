...
public class DirectIndex extends BitPostingIndex {
	...
	public static void main (String args[]) throws Exception
	{
		...
		System.out.println("docid" + args[0] + " pointer = "+ die.toString());
		IterablePosting pi = direct.getPostings(die);
		System.out.print(args[0] + " ");
		MetaIndex meta = index.getMetaIndex();
		System.out.println("按序号获取的的文档中的词项信息,文档序号："+args[0]+"\t"+meta.getItem("docno", Integer.parseInt(args[0])));
		System.out.println("(词项ID，词项频率)");
		...
		IterablePosting postings = di.getPostings((BitIndexPointer)doi.getDocumentEntry(Integer.parseInt(args[0])));
		System.out.println("文档中词项和词频，文档序号："+args[0]);
		System.out.println("词项，词频");
		while (postings.next() != IterablePosting.EOL) {
			...
			System.out.println("{{{{"+lee.getKey() + " , " + postings.getFrequency()+"\t"+
					...
					+lee.getValue().getFrequency()+"\t"
					+lee.getValue().getTermId()+"\t"
					+ postings.getId()+"}}}}");
		}
		Thread.sleep(10000);
		
		PostingIndex<?> inv = index.getInvertedIndex();
		LexiconEntry le = lex.getLexiconEntry( "gif" );
		IterablePosting postings2 = inv.getPostings((BitIndexPointer) le);
		while (postings2.next() != IterablePosting.EOL) {
			String docno = meta.getItem("docno", postings2.getId());
			System.out.println(docno + " with frequency " + postings2.getFrequency()+"\t"+postings2.getDocumentLength()
					+"\t"+postings2.getDocumentLength());
		}
