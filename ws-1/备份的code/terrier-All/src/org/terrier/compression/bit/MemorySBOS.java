 ...
 * 
 * 
 * 这个类扩展了BitByteOutputStream，因此它提供了压缩写入功能，但是
 * 使用MemoryOutputStream作为底层OutputStream，因此需要单独刷新到磁盘。
 * 它使用BitByteOutputStream而不是BitOutputStream，因为MemoryOutputStream类已经有自己的缓冲区
 * (以保持内存中的一切），并且额外的缓冲不足。
 * 
