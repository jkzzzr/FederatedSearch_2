...
/**
* Request包含搜索引擎对于一个查询的细节。
* 包括：查询本身，查询结果集，控制器和文本对象。
* 控制器设定了参数，他们经常加载后处理，和后置过滤器，或者设置匹配参数。
* 文本对象是任意的你传递给后处理和后置过滤器的对象。
* 控制器经常描述查询每个阶段用到的模块：
* 	用到的匹配和权重模型。
* 	后置处理模块将变换应用于结果集
* 	后置分类器模块将变换应用于结果集
* Request应用于SearchRequest界面。额外的，Request保存了这个查询相关的所有控制器。
